/*=============================================================================
    vp9_encoder.c — FFmpeg libavcodec VP9 video encoder implementation
=============================================================================*/
...
static zst_result_t
vp9_init_encoder(vp9_encoder_t* s, uint32_t width, uint32_t height)
{
    s->width = width;
    s->height = height;
    ...
    if (!codec) {
        fprintf(stderr, "vp9_encoder: VP9 encoder not found (libvpx-vp9 needed)\n");
        return ZST_ERROR;
    }
    ...
    s->codec_ctx->codec_id = AV_CODEC_ID_VP9;
    ...
    s->codec_ctx->gop_size = s->gop_size > 0 ? s->gop_size : 60;
...
static zst_result_t
vp9_encode_frame(vp9_encoder_t* s, const uint8_t* yuv_data, size_t yuv_size,
                 zst_buffer_t** out)
{
    if (!s->initialized) {
        if (vp9_init_encoder(s, s->width, s->height) != ZST_OK) {
            return ZST_ERROR;
        }
    }
...
vp9_process(zst_element_t* el, zst_buffer_t* in, zst_buffer_t** out)
{
    ...
    if (in->memory.size == expected_size) {
        return vp9_encode_frame(s, (const uint8_t*)in->memory.data, in->memory.size, out);
    }

    /* If input is VP9 (passthrough), just forward */
...
zst_element_t* zst_zst_vp9_encoder_create(void);

static zst_element_t*
plugin_create_element(const char* name)
{
    if (strcmp(name, "vp9enc") == 0) {
        return zst_zst_vp9_encoder_create();
    }
    return NULL;
}
