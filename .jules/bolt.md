...
**Action:** When integrating with video encoders that accept raw pixel buffers (like `x264` and `x265`), directly map the incoming buffer's plane pointers and stride array to the encoder's input structures instead of issuing `memcpy` calls. Be sure to adjust lifecycle cleanup (avoid `x264_picture_alloc`/`x264_picture_clean` which would free external memory).
